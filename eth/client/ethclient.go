...
import (
	...
	ethclientBase "github.com/ethereum/go-ethereum/ethclient"
...
type EthClient interface {
	...
	SubscribeNewHead(ctx context.Context, ch chan<- *types.Header) (sub ethereum.Subscription, err error)
}

type EthClientWithRet interface {
	EthClient

	SendTransactionWithRet(ctx context.Context, tx *types.Transaction) (txHash common.Hash, err error)
}

type ethClientWithRet struct {
	*ethclientBase.Client

	rc *rpc.Client
}

func NewEthClient(rc *rpc.Client) EthClientWithRet {
	return &ethClientWithRet{
		Client: ethclientBase.NewClient(rc),

		rc: rc,
	}
}

func (p *ethClientWithRet) SendTransactionWithRet(ctx context.Context, tx *types.Transaction) (txHash common.Hash, err error) {
...
func TransactFn(p EthClientWithRet, contractAddress common.Address, txHashOut *common.Hash) TransactFunc {
...
func (p *ethClientWithRet) SubscribeNewHead(ctx context.Context, ch chan<- *types.Header) (sub ethereum.Subscription, err error) {
	return p.Client.SubscribeNewHead(ctx, ch)
}

func (p *ethClientWithRet) ChainID(ctx context.Context) (*big.Int, error) {
